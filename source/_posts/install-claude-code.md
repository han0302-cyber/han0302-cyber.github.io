...
另外還有一種「**全放行（Bypass Permissions）**」——什麼都不問、全部照做。它刻意**不在** `Shift+Tab` 的循環裡，必須用特殊參數啟動才會出現，官方文件也把它定位為隔離環境（容器、虛擬機）專用。⚠️ 律師在放著真實資料的電腦上，不建議使用這個模式。（模式的名稱與種類隨版本更新略有調整，例如官方也在實驗更聰明的自動授權模式，以你畫面上實際顯示的為準。）
